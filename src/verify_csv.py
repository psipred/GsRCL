...
def verify(args):
    ...
    if count > 1000:
        print(
            'Error 129: The genes experssion matrix should include no more than 1000 rows (i.e. cells),' \
    ...
    if genes_experssion_mat.shape[0] > genes_experssion_mat.shape[1]:
        print(
            'Error 130: The genes experssion matrix topology should be cells x genes, where' \
    ...
    if not query_genes.dtype == 'object':
        print(
            'Error 131: The input csv file header should contain gene ids,' \
    ...
    if np.unique(query_genes).shape[0] != query_genes.shape[0]:
        print(
            'Error 132: The set of query genes has duplicates.' 
        )
        sys.exit(132)

    if not query_barcodes.dtype == 'object':
        print(
            'Error 133: The input csv file rows should start with cell barcodes,' \
    ...
    if matched_genes_mask.sum() / query_genes.shape[0] <= 0.5:
        print(
            f'Error 134: {round((matched_genes_mask.sum() / query_genes.shape[0]) * 100)}% of query genes are found in reference genes,' \
